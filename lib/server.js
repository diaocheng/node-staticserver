...
exports = module.exports = function() {
    ...
    Server.defaultConfiguration = function() {
        ...
        this.set('statusCode', require('./json/statuscode.json'));
    ...
    Server.set = function(setting, val) {
        ...
        switch (setting) {
            case 'root':
                this.config[setting] = val.replace(/\\/g, '/');
        }
        return this;
    };
        ...
        fs.stat(pathname, function(err, stats) {
            ...
            if (err) {
                ...
                if (stats.isDirectory()) {
                    ...
                    // 设置状态码为200
                    status = 200;
                    var charset = Server.config.charset ? Server.config.charset : 'utf-8';
                    // 设置响应头
                    res.writeHead(status, {
                        'Content-Type': Server.config.mime[ext] + ';charset=' + charset,
