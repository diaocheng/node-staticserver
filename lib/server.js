    ...
    fs.stat(request.filename, function(err, stats) {
        if (err) {
            ...
            if (stats.isFile()) {
                ...
                // 设置缓存
                var cache = me.getCache(request.IfModifiedSince, stats.mtime);
                header = util.merge(header, {
                    'Date': cache.Date.toUTCString(),
                    'Expires': cache.Expires.toUTCString(),
 ...
 * @return {[type]}                 [description]
 */
Server.getCache = function(IfModifiedSince, mtime) {
